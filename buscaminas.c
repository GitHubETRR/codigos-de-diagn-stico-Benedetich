        ...
        for (int c = 0; c < columnas; c++) {
            ...
            for (int dirFila = -1; dirFila <= 1; dirFila++) {
                for (int dirColumna = -1; dirColumna <= 1; dirColumna++) {
                    if (dirFila == 0 && dirColumna == 0) continue;
                    int numFila = numFila + dirFila;
                    int numColumna = c + dirColumna;
                    if (numFila >= 0 && numFila < filas && numColumna >= 0 && numColumna < columnas) {
                        fila_t* filaVecina = tablero;
                        for (int k = 0; k < numFila; k++) {
                            filaVecina = filaVecina->siguiente;
                        }
                        celda_t* celdaVecina = filaVecina->celdas;
                        for (int l = 0; l < numColumna; l++) {
    ...
    if (celdaActual->minasRededor == 0) {
        for (int dirFila = -1; dirFila <= 1; dirFila++) {
            for (int dc = -1; dc <= 1; dc++) {
                ...
                descubrirCelda(tablero, filas, columnas, f + dirFila, c + dc);
            }
        }
    }
    return false;
...
void jugarBuscaminas(int filas, int columnas, int minas) {
    ...
    while (jugando) {
        ...
        printf("\nIngrese fila y columna (0-4): \n");
        printf("El primer valor es para la fila, y el segundo para la columna, por ejemplo (0 2) \n");
        scanf("%d %d", &f, &c);
        if (f < 0 || f >= filas || c < 0 || c >= columnas) {
...
int main() {
    srand(time(NULL));
    jugarBuscaminas(Minas_Filas_Columnas, Minas_Filas_Columnas, Minas_Filas_Columnas);
    return 0;
}
